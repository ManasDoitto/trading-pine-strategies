---
name: journal
description: Build the option-buying trade journal for a day. It pulls Dhan fills read-only, computes FIFO P&L, flags rule violations (averaging down, overnight/expiry holds, premium stop, daily loss, revenge, deep OTM, sell-to-open), and writes the markdown journal with TradingView snapshots. Use for /journal, "update my journal", "journal today", or as the first step of a session-close run.
---

# /journal [YYYY-MM-DD] [NSE|MCX|ALL]

Arguments are optional. Date defaults to today; session defaults to ALL (it only controls which charts get snapshotted).

1. **Build the facts (deterministic, no LLM numbers).** From the repo root run:
   ```bash
   ...
   - `--offline` makes no Dhan calls at all.
...
2. **Write the journal.** Spawn the `trade-journal-keeper` subagent with a prompt that gives it
   the facts path printed in step 1 (`journal_data/facts/<date>_journal.json`) and today's date.
   It writes `journal_data/journal/<date>.md` and handles snapshots.
